...
const Skills = () => {
  ...
  // Dono libraries mein dhundega icon
  const getIcon = (iconName) => {
    if (SiIcons[iconName]) return SiIcons[iconName]; // pehle Si mein check karo
    if (Icons[iconName]) return Icons[iconName];      // phir lucide mein
    return Icons.Code2;                               // fallback
  };
            ...
            <FadeIn delay={categoryIndex * 100} key={category}>
              <div className="relative bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-primary/30 transition-all duration-300 group">
                <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/10">
                  <div className="w-1 h-8 bg-linear-to-b from-primary/20 to-primary/10 rounded-full"></div>
